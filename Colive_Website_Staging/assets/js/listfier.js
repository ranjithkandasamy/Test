        ...
        init: function (options) {
            ...
            filter
                .change(function () {
                    ...
                    that.items.hide();
                    if (filtertext.length < 1 && that.settings.filter.attr('id') === 'txtSearchFilter') {
                        $.each(that.items, function (i, v) {
                            var objElement = $(this).parent().parent().parent().parent();
                            if (objElement.hasClass('collapse')) {
                                //if (objElement.hasClass('show')) {
                                //    objElement.removeClass('show');
                                //}
                            }  
                        });
                    } else if (filtertext.length < 1 && that.settings.filter.attr('id') !== 'txtSearchFilter') {
                        $.each(that.items, function (i, v) {
                            var objElement = $(this).parent().parent().parent().parent();
                            if (objElement.hasClass('collapse')) {
                            ...
                            var objNxtElement = $(this).parent().parent().parent().parent().next();
                            if (objNxtElement.hasClass('collapse')) {
                                objNxtElement.addClass('show');
                            } 
                        });
                    }
                    ...
                    match.show();
                    if (filtertext.length >= 1 && that.settings.filter.attr('id') === 'txtSearchFilter') {
                        var objParent = match.parent().parent().parent().parent();
                        if (objParent.hasClass('collapse')) {
                            if (objParent.hasClass('show') === false) {
                                objParent.addClass('show');
                            }
                        }
                    } 
